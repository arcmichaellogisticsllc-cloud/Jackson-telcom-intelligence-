...
$add('WARN', 'executive doctrines missing', $count('SELECT CASE WHEN COUNT(*) < 5 THEN 1 ELSE 0 END FROM executive_doctrines WHERE active = 1'));
$add('WARN', 'executive packages without doctrine evaluation', $count('SELECT COUNT(*) FROM executive_packages ep LEFT JOIN doctrine_evaluations de ON de.entity_type = "Executive Package" AND de.entity_id = ep.id WHERE de.id IS NULL'));
$add('WARN', 'opportunities without doctrine evaluation', $count('SELECT COUNT(*) FROM opportunities op LEFT JOIN doctrine_evaluations de ON de.entity_type = "Opportunity" AND de.entity_id = op.id WHERE de.id IS NULL'));
$add('WARN', 'regions without executive health score', $count('SELECT COUNT(*) FROM regions r LEFT JOIN executive_health_scores ehs ON ehs.region_id = r.id WHERE ehs.id IS NULL'));
$add('WARN', 'quarterly reviews missing', $count('SELECT CASE WHEN COUNT(*) = 0 THEN 1 ELSE 0 END FROM quarterly_reviews'));
